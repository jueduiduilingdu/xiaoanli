    ...
    protected void onDraw(Canvas canvas) {
        ...
        for(ClipInfo clipInfo : mClipInfoList){
            ...
            switch (clipInfo.clipType){
                case ClipInfo.CLIP_TYPE_SPACE:
                    canvas.drawRect(totalWidth - getResources().getDimension(R.dimen.margin_1), 0f, newWidth, getHeight(), mSpacePaint);
                    break;
        ...
        if(lastTotalProgress + mCurClipInfo.progress < mMinDuration){
            canvas.drawRect(mMinDuration / (float)mMaxDuration * getWidth(), 0f,
                    mMinDuration / (float)mMaxDuration * getWidth() + getResources().getDimension(R.dimen.margin_2), getHeight(), mSpacePaint);
        }
        if(isCursorShow || isInProgress){
            canvas.drawRect(totalWidth, 0f, totalWidth + getResources().getDimension(R.dimen.margin_2), getHeight(), mSpacePaint);
        }
    }
